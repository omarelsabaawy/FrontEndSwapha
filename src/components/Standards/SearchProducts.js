...
function SearchProducts() {
    ...
    const { country, query } = useParams();

    useEffect(() => {
        const fetchPages = async () => {
            try {
                setPagesLoading(true);
                const productsNumber = await axios.get(`/api/products/search/numbers/${country}?query=${query}`);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const result = await axios.get(`/api/products/search/${country}?query=${query}&page=${currentPage}&perPage=16`);
    ...
    return (
        <div>
            <Breadcrumb type={`Search for ${query}`} />
